...
import project2.SpringMVC.utiles.TimeParse
      ...
      rdd.foreachPartition(partitionRecords  => {
        ...
        try{
         ...
         partitionRecords.foreach(record => {
           ...
           //按月建立一张新表存储数据
           var sql = s"CREATE TABLE if not exists `word_count_${TimeParse.timeStamp2String(createTime, "yyyyMM")}`(" +
                           s"`id` int(11) NOT NULL AUTO_INCREMENT," +
                           s"`word` varchar(64) NOT NULL," +
                           s"`count` int(11) DEFAULT '0'," +
                           s"`date` date NOT NULL, PRIMARY KEY (`id`), " +
                           s"UNIQUE KEY `word` (`word`,`date`) " +
         ...
         log.warn(s"[保存的批处理操作完成] 耗时: ${System.currentTimeMillis - preTime}")
          ...
          case e:Exception =>  log.error("[保存的批处理操作失败]", e)
        }finally {
